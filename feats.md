...
## Phase 1: settings and browser-data import (shipped)
...
- [x] Accept password CSV exports with URL, username, and password columns. Write passwords only to the macOS Keychain; show site/account names in the preview, never password values.
...
- Pause blocking for the selected hostname, manage paused sites, and apply the choice before main-frame navigation and reload affected open tabs so the updated rule list applies to their next requests. Exceptions match the exact hostname.
...
- Reuse the shipped Phase 1 CSV importer; preserve exact scheme/host/port matching and show site/account names, never password values.
...
1. Extend the shipped Settings importer only if additional formats are needed.
