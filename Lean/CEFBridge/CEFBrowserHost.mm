...
std::string YouTubeAdSkipScript() {
  ...
  return "(()=>{try{if(window.__leanYtAdsEnabled===undefined)"
         ...
         "v.currentTime<v.duration-0.5){try{v.dataset.leanAdSeek='1';"
         "v.currentTime=v.duration-0.2;}catch(e){}}"
         "}else{if(v.dataset.leanMuted){v.muted=false;delete v.dataset.leanMuted;}"
         "if(v.dataset.leanAdSeek){const p=q('#movie_player');"
         "if(v.readyState>=2){delete v.dataset.leanAdSeek;delete v.dataset.leanKickCount;"
         "try{if(p&&p.seekTo){p.seekTo(0,true);}}catch(e){}}"
         "else{const n=Number(v.dataset.leanKickCount||0);"
         "if(n<12){v.dataset.leanKickCount=String(n+1);"
         "try{if(p&&p.seekTo){p.seekTo(n%2===0?5:0,true);}}catch(e){}}"
         "else{delete v.dataset.leanAdSeek;delete v.dataset.leanKickCount;}}}}};"
