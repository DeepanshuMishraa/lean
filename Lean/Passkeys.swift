...
//
// Ported from Search's Sources/Search/Passkeys.swift (Sep 2026), adapted to
// Lean's setting storage and content-world setup. The design is unchanged:
//
...
// If Lean dies meanwhile — quit, crash, killed — the agent never lets go of
// it, and refuses every passkey request Lean makes after that, before any
...
// on every site, until the agent restarts with the Mac.
//
// So Lean carries the ceremony itself, as Chrome and Firefox do on the Mac,
...
// answer; what it leaves to Lean just waits, as it does while nobody picks
// one, never reaching macOS.
//
// What Lean needs from Apple for this to work: the restricted entitlement
// `com.apple.developer.web-browser.public-key-credential`, granted to
// browsers on request, with its matching Developer ID provisioning profile
// embedded (see Lean.passkeys.entitlements). Without it the switch in
// Settings defaults off and pages fall back to passwords.
@MainActor
final class Passkeys: NSObject {
    ...
    /// Wired to `LeanStore.passkeysEnabled`, which defaults on. Reaching
    /// the Mac's sheet needs Apple's browser entitlement; builds without
    /// it hand requests over all the same, and sites fall back to
    /// passwords when macOS refuses.
    static var isEnabled = isEntitled

    /// Set in unit tests so no system sheet can ever appear: ceremonies are
    /// answered with a made-up credential instead.
    static var rehearsesForTests = false

    // MARK: - the Mac's permission

    /// Whether this Mac lets Lean use its passkeys at all: a question
    ...
    /// Whether this running process carries Apple's browser entitlement —
    /// asked of its own signature, which is the only thing that decides it.
    /// An ad-hoc build has neither the entitlement nor a profile, so this is
    /// false there and passkeys stay off.
    ...
    private static func ensure(_ then: @escaping () -> Void) {
        guard isEntitled, access == .notDetermined else { return then() }
        if waiting != nil {
            waiting?.append(then)
            return
        }
    ...
    /// How many a page has asked for, and the last as it was checked.
    ...
    func perform(_ body: [String: Any], from caller: Caller, answer: @escaping ([String: Any]) -> Void) {
        ...
        // Lean itself behind doesn't get to bring up the Mac's sheet over
        // what you're looking at. A test rehearsal is always behind.
        guard Passkeys.rehearsesForTests || (NSApp.isActive && caller.window?.isKeyWindow == true) else {
        ...
        // A test rehearsal never shows the sheet: it would open on the screen
        // of whoever is working beside it. It gets a credential made up on the
        // spot instead, so the checks above and the page's side are still
        // what they are for real.
        if Passkeys.rehearsesForTests {
    ...
    nonisolated static func failure(_ name: String, _ message: String) -> [String: Any] {
        ["error": name, "message": message]
    }

    nonisolated static func assertionReply(id: Data, clientData: Data, authenticatorData: Data, signature: Data, user: Data, attachment: String) -> [String: Any] {
        [
    ...
    nonisolated static func registrationReply(id: Data, clientData: Data, attestation: Data, transports: [String], attachment: String) -> [String: Any] {
    ...
    /// For a test rehearsal: a credential in the shape the sheet gives, made
    /// up from the request — a P-256 key and all, with nothing signed.
    nonisolated private static func rehearsal(_ kind: String, rp: String, origin: String, challenge: Data) -> [String: Any] {
    ...
    nonisolated static func data(_ value: Any?) -> Data? {
    ...
    nonisolated private static func descriptors(_ value: Any?) -> [(id: Data, transports: [ASAuthorizationSecurityKeyPublicKeyCredentialDescriptor.Transport])] {
    ...
    nonisolated private static func verification(_ value: Any?) -> ASAuthorizationPublicKeyCredentialUserVerificationPreference {
    ...
    nonisolated private static func attestation(_ value: Any?) -> ASAuthorizationPublicKeyCredentialAttestationKind {
        switch value as? String {
    ...
    nonisolated static func authenticatorData(inAttestation object: Data) -> Data? {
    ...
    nonisolated static func publicKey(inAuthenticatorData data: Data) -> (algorithm: Int, der: Data?)? {
...
/// The world Lean's own page scripts are reached through. The passkey patch
/// has to stand in the page's own world — it replaces the page's functions —
/// and reaches Lean through window events picked up by a bridge here, where
/// the handler lives and no page can look. Only this bridge uses the world
/// so far; Lean's other scripts stay where they were.
enum LeanWeb {
    @MainActor static let world = WKContentWorld.world(name: "Lean")
}
...
final class PasskeyRelay: NSObject, WKScriptMessageHandlerWithReply {
    static let name = "leanPasskeys"
    /// The page's word to Lean's side, and the answer back (see `bridge`).
    static let asked = "lean-passkeys-ask"
    static let answered = "lean-passkeys-answer"
    ...
    /// functions, and nothing of Lean's is there for a page to find — no
    /// window.webkit, no global of its own. Requests go to Lean's side as
    ...
    (function () {
      ...
      // In once, whichever copy comes first — an extension's or Lean's own —
      // and marked on the prototype rather than on the window. The mark holds
      // navigator.credentials too: held, WebKit keeps it, and an extension's
      // own get and create on it with it.
      var mark = Symbol.for('lean.passkeys');
      ...
      // A password manager that keeps passkeys puts its own get and create on
      // navigator.credentials, or asks from its own script, and offers its
      // passkeys under the name field to the sites that ask for them that
      // way. Once one is there, pages hear the field can.
      var claimed = false;
    ...
    /// Lean's side of it, in Lean's own world (LeanWeb.world), where the
    ...
    (function () {
      ...
      if (!handler || window.__leanPasskeysBridged) return;
      window.__leanPasskeysBridged = true;
    ...
    /// When passkeys are off: only the passkey object goes.
    /// navigator.credentials itself stays: sites use it for stored passwords
    /// too, and that half still works.
    ///
    /// Unless an extension answers passkey requests itself — a password
    /// manager with your passkeys in it. It puts its own get and create on
    /// navigator.credentials, and reaches for the passkey object from its own
    /// script as it does; from then on sites see the object, and the
    /// extension is the one they ask. Whatever it leaves to the browser is
